 ...
 */
class AdminGeneralSettingsController extends AbstractController
{
    ...
    /**
     * @Route("/settings/admin-page", name="settings-admin-general")
     *
     * Param $storeSettingsDirectory comes from services.yaml
     * Param $adminSettingsFile comes from services.yaml
     */
    public function editSettings(Request $request, StoreSettings $settings, string $storeSettingsDirectory, string $adminSettingsFile)
    {
        $configDirectories = [$storeSettingsDirectory];

        $fileLocator = new FileLocator($configDirectories);
        $locatedFile = $fileLocator->locate($adminSettingsFile, null, false);

        if (count($locatedFile) === 1) {
            $values = Yaml::parseFile($locatedFile[0]);
        } else {
            throw new Error( sprintf('HIBA: Multiple %s files were found. Make sure you have only one %s file in your %s folder!', $adminSettingsFile, $storeSettingsDirectory));
        }
...
//        dd($form);
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('settings-admin-general');
        }
        
        $title = 'Beállítások';
        return $this->render('admin/settings/admin-general.html.twig', [
